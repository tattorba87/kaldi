...
# Note parse_options.sh can be found because  path.sh was called to add it to the path.
# As the name implies it is a generic script to parse the options.
if [ -f path.sh ]; then . ./path.sh; fi
. parse_options.sh || exit 1;

# If the number of arguments is incorrect, display this help and exit
if [ $# -lt 1 ] || [ $# -gt 3 ]; then
...
#utils/validate_data_dir.sh --no-text --no-feats --no-spk-sort $data || exit 1;
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    compute-mfcc-feats  $vtln_opts --verbose=2 --config=$mfcc_config scp,p:$logdir/wav_${name}.JOB.scp ark:- \| \
    copy-feats $write_num_frames_opt --compress=$compress ark:- ark,scp:$mfccdir/raw_mfcc_$name.JOB.ark,$mfccdir/raw_mfcc_$name.JOB.scp \
    || exit 1;
fi
